...
(function($) {
	...
	var xmlrpc = $.xmlrpc = function(url, settings) {
		...
		if ("XMLSerializer" in window) {
			settings.data = {
				xmlrpcMsg : new window.XMLSerializer().serializeToString(xmlDoc),
		...
		// HAXXXX
		settings.data.xmlrpcMsg = settings.data.xmlrpcMsg.replace(/(<int>)(\d+)(<\/int>)/g, '<double>$2</double>');
